...
#### 5. 订单识别与绩效计算
- ✅ 自动识别订单：门店总结算金额=33.95元/36.86元/48.5元（±0.01）计为1单
- ✅ 按单收取：固定7元/单
- ✅ 实时显示总订单数和绩效总额
- ✅ 动态显示/隐藏
...
- **订单识别规则**：当门店总结算金额为33.95元、36.86元或48.5元（允许±0.01元浮动）时，计为1单
- **绩效标准**：固定7元/单
- **计算公式**：绩效总额 = 总订单数 × 7元
- **实时显示**：自动统计总订单数和绩效总额
...
// TIER_CONFIG 在主脚本中集中维护档位金额与绩效系数
// 匹配任意配置档位（允许±0.01元误差）即视为1单
const tierMatch = Object.values(TIER_CONFIG).find(tier => Math.abs(amount - tier.amount) < 0.01);
const orderCount = tierMatch ? 1 : 0;
```
