        ...
        reader.onload = function(e) {
            try {
                ...
                if (processedData.length > 0) {
                    ...
                    if (!firstRow['门店id']) {
                        throw new Error('Excel文件缺少"门店id"字段');
                    }
                    if (!firstRow['代运营结算金额']) {
                        throw new Error('Excel文件缺少"代运营结算金额"字段');
                    }
                }
...
function analyzeStoreData(data, storeIdList) {
    ...
    storeIdList.forEach(storeId => {
        ...
        // 计算统计数据
        const storeName = storeData[0]['门店名称'] || '未知商家';

        // 计算结算天数（按账单日期去重）
        const settlementDays = new Set(
            storeData.map(row => {
                const date = row['账单日期'];
        ...
        const amount = storeData.reduce((sum, row) => {
            const settlementAmount = parseFloat(row['代运营结算金额']) || 0;
    ...
    console.log(`总结算天数: ${results.totalDays}`);
        ...
        <table class="results-table">
            ...
            <tbody>
                ...
                <tr class="summary-row">
                    ...
                    <td><strong>${results.totalDays}</strong></td>
...
function exportReport() {
    ...
    try {
        ...
        exportData.push({
            '门店ID': '查询门店总数',
            '门店名称': analysisResults.totalStores,
            '结算天数': '',
            '结算金额': '',
            '记录数': '',
            '状态': ''
        ...
        exportData.push({
            '门店ID': '找到数据门店',
            '门店名称': analysisResults.foundStores.length,
            '结算天数': '',
            '结算金额': '',
            '记录数': '',
            '状态': ''
        ...
        exportData.push({
            '门店ID': '总结算天数',
            '门店名称': analysisResults.totalDays,
            '结算天数': '',
            '结算金额': '',
            '记录数': '',
            '状态': ''
        ...
        exportData.push({
            '门店ID': '结算金额汇总',
            '门店名称': `¥${analysisResults.totalAmount.toFixed(2)}`,
            '结算天数': '',
            '结算金额': '',
            '记录数': '',
            '状态': ''
        ...
        XLSX.utils.book_append_sheet(workbook, worksheet, "饿了么门店统计报告");
        ...
        const filename = `饿了么门店统计报告_${timestamp}.xlsx`;
...
function resetAll() {
    ...
    resultsContainer.innerHTML = `
        <div class="empty-state">
            ...
            <small>支持 .xls 和 .xlsx 格式的饿了么周期账单</small>
        </div>
    `;
